...
def _get_mapping(cats, filter_cats, parent=None):
    for cat in cats:
        ...
        if cat["id"] in filter_cats:
            parent = cat["id"]
        yield cat["id"], parent
        yield from _get_mapping(cat.get("categories", []), filter_cats, parent=parent)
...
def make_cache_key(params, method="search"):
    ...
    return "froide_food:foursquare:%s:%s" % (
        method,
        "_".join(str(params.get(key)) for key in keys),
    )
...
class FoursquareVenueProvider(BaseVenueProvider):
    ...
    def get_places(
        ...
        **kwargs
        ...
        if location is not None:
            ...
            params.update({"ll": "{},{}".format(*coordinates)})
        if q:
            can_cache = False
            params["query"] = q
        ...
        if not can_cache or response is None:
            ...
            logger.info(
                "API Request: %s (%s)", response.request.url, response.status_code
            )
    ...
    def get_place(self, ident):
        ...
        response = requests.get(
            LOOKUP_URL.format(ident=ident), params=params, headers=HEADERS
        )
        if response.status_code != 200:
