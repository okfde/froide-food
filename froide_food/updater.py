...
from .utils import merge_venues, names_similar


class FoodUpdater:
    def get_used_osm_ids(self):
        idents = VenueRequest.objects.filter(ident__startswith="amenity:").values_list(
            "ident", flat=True
        )
        ...
        for i, osm_id in enumerate(used_existing_osm_ids):
            ...
            if venue.name != amenity.name:
                if not names_similar(venue.name, amenity.name):
                    print(venue.name, amenity.name, venue.ident, amenity.osm_id)
                    continue
            safe_update_osm_ids.add(osm_id)
